...
export const playAlongsService: SceneService = {
  getAll: function ({ page, filters, sort, signal }) {
    return call({
      url: `/musora-api/all?included_types[]=play-along&statuses[]=published&statuses[]=scheduled&future&sort=${
        sort || '-published_on'
  ...
  getInProgress: function ({ page, filters, sort, signal }) {
    return call({
      url: `/musora-api/in-progress?included_types[]=play-along&limit=40&sort=${
        sort || '-published_on'
      }&page=${page || 1}${filters || ''}`,
      signal
    });
  ...
  getCache: async function () {
    return JSON.parse((await AsyncStorage.getItem('@playAlongs')) || '{}');
  }
};
